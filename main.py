            ...
            async with asyncio.timeout(120):
                while True:
                    ...
                    if not raw:
                        ...
                        raise RuntimeError(
                            f"[engine-{self.eid}] exited before READY. "
            ...
            while True:
                ...
                if line.startswith("TOKEN"):
                    ...
                    try:
                        ...
                        continue
                    if decoded_text:
                        yield {"type": "token", "id": token_id, "text": decoded_text, "elapsed_ms": elapsed}
...
class EnginePool:
    ...
    def status(self):
        return [
            {"engine_id": i, "pid": self.engines[i].pid,
...
class SessionData:
    ...
    def new_turn_tokens(self, user_msg: str) -> list:
        if self.tokens_in_engine == 0:
            text = f"{SYSTEM_TOKEN} {self.system_prompt}{SEP}{USER_TOKEN} {user_msg}{SEP}{ASST_TOKEN} "
        else:
...
_metrics_lock = None
metrics = {
    "total_requests": 0, "total_tokens": 0, "total_ms": 0.0,
    "errors": 0, "start_time": time.time(), "sessions_evicted": 0,
}
...
async def session_gc_loop():
    while True:
        ...
        if not _metrics_lock:
            continue
        now     = time.monotonic()
        expired = [sid for sid, s in list(sessions.items()) if now - s.last_active > SESSION_TTL_S]
        for sid in expired:
...
def _download_file(repo_id: str, filename: str, dest_dir: Path) -> None:
    ...
    if not dest.exists():
        raise FileNotFoundError(f"Copy failed, {dest} missing.")
    print(f"[HF HUB] {filename} ready ({dest.stat().st_size // 1_000_000} MB)")


async def _startup_background():
    loop = asyncio.get_running_loop()
    if MODEL_BIN.exists():
        print(f"[startup] model.bin present ({MODEL_BIN.stat().st_size // 1_000_000} MB) - skip download")
    else:
        try:
            await loop.run_in_executor(None, _download_file, HF_REPO_ID, "model.bin", BASE_DIR)
    ...
    if TOKENIZER_BIN.exists():
        ...
        try:
            await loop.run_in_executor(None, _download_file, HF_REPO_ID, "tokenizer.bin", BASE_DIR)
        except Exception as e:
...
async def pool_status():
    return {"n_engines": N_ENGINES, "omp_per_engine": OMP_PER_ENGINE,
            "engines": pool.status(), "sessions": len(sessions)}
...
async def chat(req: ChatRequest):
    ...
    if sess.tokens_in_engine + len(new_tokens) + req.max_new_tokens > MAX_SESSION_TOKENS:
        ...
        new_tokens = sess.rebuild_with_sliding_window(req.message, MAX_SESSION_TOKENS - req.max_new_tokens)
    ...
    async def event_stream():
        ...
        tokens_generated = 0
        try:
            async for chunk in pool.generate(req.session_id, new_tokens, req.max_new_tokens, req.temperature, req.top_k):
                if chunk["type"] == "token":
                    tokens_generated += 1
            ...
            if final_chunk is not None and not stopped_early:
                ...
                tps      = round(tok_out / (total_ms / 1000.0), 2) if total_ms > 0 else 0
                tokens_generated = tok_out
            sess.append_assistant(reply)
            async with _metrics_lock:
                metrics["total_tokens"] += tok_out
                metrics["total_ms"]     += elapsed
            yield f"data: {json.dumps({'type': 'done', 'total_tokens': tok_out, 'total_ms': total_ms, 'tps': tps, 'session_id': req.session_id, 'full_response': reply})}\n\n"
        except Exception as e:
            async with _metrics_lock:
                metrics["errors"] += 1
